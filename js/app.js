(function() {
    ...
    const INIT_SCORE = 10*60*1000; // 5' to ms
    const RECORDS_NAME = 'mm_puzzle';
    const INPUT_ID = 'user-name';
    const API_URL_RANDOM_DOGS = 'https://api.thedogapi.com/v1/images/search?limit=1&mime_types=jpg&size=med'
    const API_URL_RANDOM_CATS = 'https://api.thecatapi.com/v1/images/search?limit=1&mime_types=jpg&size=med'
    ...
    const puzzle = window.puzzle = {
        ...
        /**
         * Puzzle URL_Imagen (Image to be squared)
         * @type {String} 
         */
        URL_Image: '',
         ...
         */
        setupTile: function(coords, tileCounter, url_image = this.URL_Image) {
            const tile = this.uiHelper.createTile(coords, tileCounter, url_image);
    ...
    window.onload = function() {
        ...
        const btnOtheDog = document.querySelector("#btnOtherDog")
        const btnOtheCat = document.querySelector("#btnOtherCat")
        const resetGame = document.querySelector("#resetGame")
        let radioCHecked = document.querySelectorAll('input[name="radio"]')
        radioCHecked.forEach(radio => {
            radio.addEventListener('click', () => {
                puzzle.init(Number(radio.value));
                piezas = Number(radio.value)
                const activeRadio = document.querySelector('.active')
                const placeHolderGrid = document.querySelector('.Puzzle-placeholder')
                activeRadio.classList.remove('active')
                radio.parentElement.classList.add('active')
                placeHolderGrid.src = puzzle.URL_Image
            })
        });
        btnOtheDog.addEventListener('click', () => {
            loadRandomPerros()
        })
        btnOtheCat.addEventListener('click', () => {
            loadRandomGatos()
        })
    ...
    async function loadRandomPerros() {
        const res = await fetch(API_URL_RANDOM_DOGS)
        const data = await res.json()
    
        if (res.status !== 200) {
            alert('Error: ' + res.status)
        } else {
            puzzle.URL_Image = data[0].url
            puzzle.init(piezas)
            const placeHolderGrid = document.querySelector('.Puzzle-placeholder')
            placeHolderGrid.src = data[0].url
        }
    }
    async function loadRandomGatos() {
        const res = await fetch(API_URL_RANDOM_CATS)
        const data = await res.json()
